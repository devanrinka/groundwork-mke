...
async function extractProjects(articles) {
  ...
  const prompt = `You are a development intelligence assistant for an architecture firm in Milwaukee, WI.

Review these news articles and extract real estate development or construction projects within ~50 miles of Milwaukee, WI.
...
function buildEmailHTML(projects, editionDate) {
  const TYPE = {
  ...
  <div style="max-width:600px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px;">
      <div>
        ...
        <div style="font-size:10px;color:#aaa;letter-spacing:2.5px;text-transform:uppercase;margin-top:5px;font-weight:300;">Milwaukee development intelligence</div>
      </div>
    ...
    <div style="border-top:1px solid #e5e5e5;border-bottom:1px solid #e5e5e5;padding:9px 0;margin-bottom:20px;font-size:12px;color:#888;">
      ${projects.length} new projects &nbsp;&middot;&nbsp; 50-mile Milwaukee radius &nbsp;&middot;&nbsp; Auto-updated
    </div>
    ${cards}
    ...
    <div style="text-align:center;font-size:11px;color:#bbb;margin-top:16px;padding-bottom:8px;">
      Groundwork &middot; Milwaukee, WI &middot; Auto-generated development intelligence
    </div>
  </div>
</body>
</html>`;
...
async function run() {
  ...
  console.log('Sending to Gemini for project extraction...');
  const projects = await extractProjects(articles);
  console.log(`Qualifying projects found: ${projects.length}`);

  if (projects.length < MIN_PROJECTS) {
    console.log(`Below minimum threshold of ${MIN_PROJECTS}. No edition sent.`);
    return;
  }
  ...
  const html = buildEmailHTML(projects, editionDate);
  const { error } = await resend.emails.send({
    from: 'Groundwork <onboarding@resend.dev>',
    to: RECIPIENT_EMAIL,
    subject: `Groundwork · ${editionDate} · ${projects.length} new projects`,
    html,
  });
